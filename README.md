...
**The phone screen is a real UI, not a diagram.** Home shows the launcher's icon grid (brand colors, a green dot on every app whose process is alive). Tap an icon and the launch travels for real: touch → InputDispatcher in system_server → the launcher files an Intent → City Hall approves → Zygote forks → a ward rises → its first frame lands back on the glass. **◁ Back** pops the Activity's back stack, then finishes it at the root (process stays cached). **○ Home** backgrounds it. **▢ Recents** shows one task card per live process — tap to hot-start it. Background fetches post **notifications**: a dot on the status bar, a row in the shade, and tapping it fires the PendingIntent (which outlives the process — a killed app cold-starts from its own notification). The camera's first launch raises a **runtime-permission dialog** — a system surface the app cannot draw or auto-accept; Allow/Deny is recorded by PackageManager.
